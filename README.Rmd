...
For `model.outcome` and `model.propensity` methods, you can input either a single formula or a list of formulas of the models in **single treatment analysis** and only one single model in each input for in **multiple treatments analysis**. When specifying a model, it is essential to define the arguments in the corresponding function as per its original use, except for data, x, y, and formula, which are pre-specified.
